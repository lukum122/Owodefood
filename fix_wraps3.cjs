...
function wrapFunctionRegex(regex) {
  const match = content.match(regex);
  if (!match) {
    console.log('Could not find match for regex: ' + regex);
    return;
  }
...
wrapFunctionRegex(/const register = async \([\s\S]*?\) => \{/);
wrapFunctionRegex(/const placeOrder = async \([\s\S]*?\) => \{/);
